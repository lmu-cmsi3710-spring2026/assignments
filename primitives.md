...
_Implement a two-color radial gradient for rectangles:_ Implement a `fillRectWithHalfRadialGradient` function in the _primitives_ module so that it takes two colors `c1` and `c2` and uses those colors to fill a rectangle with a semicircular _radial gradient_, with the center of the semicircle being the bottom-center of the rectangle. Its diameter should be the _smaller_ of the rectangle’s two dimensions. For example, the radial gradient for a 100×40 rectangle will have a diameter of 40 pixels; the radial gradient for a 30×98 rectangle will have a diameter of 30 pixels.
